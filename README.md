...
- Never commit `.env`, API keys, Discord tokens, database passwords, or other credentials.
- Keep `.env` user-owned and protected; use `.env.example` only as a placeholder template.
- Use a least-privilege database account for runtime access.
...
- Schema dump for the database is included at `schema.sql` in this repository (or generated by the maintainer).

### Schema and tables

The code expects the following tables (see `schema.sql` for the exact CREATE statements):

- `usage` — records queries and responses plus metadata
- `usage_images` — stores image blobs and metadata with a FK to `usage`

You can consult `schema.sql` for the complete table definitions and character-set options.
